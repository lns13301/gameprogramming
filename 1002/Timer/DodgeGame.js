...
function onReady(){
    ...
    intPlayerX = 350;
    intPlayerY = 250;
    while(arrMissile.length != 0){
        arrMissile.pop();
    }
}

function MoveMissile(){
    for(var i = 0; i < arrMissile; i++) {
        arrMissile[i].x += arrMissile[i].go_x * 3;
        arrMissile[i].y += arrMissile[i].go_y * 3;
        if (IsCollisionWithPlayer(arrMissile[i].x, arrMissile[i].y)) {
            onGameOver();
        }
    }
